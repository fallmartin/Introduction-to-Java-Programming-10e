...
public class BitOutputStream implements AutoCloseable {
    ...
    public static void main(String[] args)
        ...
        try (
                ...
                = new BitOutputStream(new File("Exercise17_17.dat"))) {
            output.writeBit("010000100100001001101");
        }

        // Try to read the data back in to verify that the program works
        try (FileInputStream input
                = new FileInputStream("Exercise17_17.dat");) {
            int r;
            while ((r = input.read()) != -1) {
                // Reading one byte at a time and display its formatted binary string
                String s = String.format("%8s",
                        Integer.toBinaryString(r & 0xFF)).replace(' ', '0');

                // Add a space within the quotes for digit grouping
                System.out.print(s + " ");
            }
            System.out.println();
        }
