 ...
 * start with: - 4 for Visa cards - 5 for Master cards - 37 for American Express
 * cards - 6 for Discover cards
 *
 ...
 *
 * Return true if the card number is valid public static boolean isValid(long
 * number)
 *
 * Get the result from Step 2 public static int sumOfDoubleEvenPlace(long
 * number)
 *
 * Return this number if it is a single digit, otherwise, return the sum of the
 * two digits public static int getDigit(int number)
 *
 * Return sum of odd-place digits in number public static int sumOfOddPlace(long
 * number)
 *
 * Return true if the digit d is a prefix for number public static boolean
 * prefixMatched(long number, int d)
 *
 ...
 * reading the input as a string and processing the string to validate the
 * credit card.)
 *
 * See text book for sample runs.
 *
 * @author martinfall
 */
public class CreditCardNumberValidation {
    ...
    public static int sumOfOddPlace(long number) {
        ...
        for (int i = getSize(number) - 1; i >= 0; i -= 2) {
            sum += getDigit(Integer.parseInt(str.charAt(i) + ""));
        }
        return sum;
    }
