
/**
 * (Geometry: the Circle2D class) Define the Circle2D class that contains:
 *
 * ■ Two double data fields named x and y that specify the center of the circle
 * with getter methods.
 * ■ A data field radius with a getter method.
 * ■ A no-arg constructor that creates a default circle with (0, 0) for (x, y) and 1
 * for radius.
 * ■ A constructor that creates a circle with the specified x, y, and radius.
 * ■ A method getArea() that returns the area of the circle.
 * ■ A method getPerimeter() that returns the perimeter of the circle.
 * ■ A method contains(double x, double y) that returns true if the
 * specified point (x, y) is inside this circle (see Figure 10.21a).
 * ■ A method contains(Circle2D circle) that returns true if the specified
 * circle is inside this circle (see Figure 10.21b).
 * ■ A method overlaps(Circle2D circle) that returns true if the specified
 * circle overlaps with this circle (see Figure 10.21c).
 *
 * Draw the UML diagram for the class and then implement the class. Write a test
 * program that creates a Circle2D object c1 (new Circle2D(2, 2, 5.5)),
 * displays its area and perimeter, and displays the result of c1.contains(3,
 * 3), c1.contains(new Circle2D(4, 5, 10.5)), and c1.overlaps(new
 * Circle2D(3, 5, 2.3)).
 *
 * @author martinfall
 */
public class Circle2D {

    // Two double data fields named x and y that specify the center of the circle 
    // with getter methods.
    private double x;
    private double y;

    public double getY() {
        return y;
    }

    public double getRadius() {
        return radius;
    }

    // A data field radius with a getter method.
    private double radius;

    public double getX() {
        return x;
    }

    /**
     * A no-arg constructor that creates a default circle with (0, 0) for (x, y)
     * and 1 for radius.
     */
    public Circle2D() {
        this(0, 0, 1);
    }

    /**
     * A constructor that creates a circle with the specified x, y, and radius.
     */
    public Circle2D(double x, double y, double radius) {
        this.x = x;
        this.y = y;
        this.radius = radius;
    }

    /**
     * A constructor that creates a circle with the specified x, y, and default
     * radius.
     */
    public Circle2D(double x, double y) {
        this(x, y, 1);
    }

    /**
     * A method getArea() that returns the area of the circle.
     *
     * @return
     */
    public double getArea() {
        return Math.pow(getRadius(), 2) * Math.PI;
    }

    /**
     * A method getPerimeter() that returns the perimeter of the circle.
     *
     * @return
     */
    public double getPerimeter() {
        return 2 * Math.PI * getRadius();
    }

    /**
     *
     *
     * A method contains(double x, double y) that returns true if the specified
     * point (x, y) is inside this circle (see Figure 10.21a).
     *
     * @param x
     * @param y
     * @return
     */
    public boolean contains(double x, double y) {
        double x1 = x;
        double y1 = y;
        double x2 = getX();
        double y2 = getY();

        // Distance between 2 points
        double distance = distance(x1, y1, x2, y2);
        // Distance between the 2 radii + the radius of the contained circle 
        // <= radius of container circle
        return distance + new Circle2D(x, y).getRadius() <= getRadius();
    }

    /**
     * A method contains(double x, double y) that returns true if the
     * specified point (x, y) is inside this circle (see Figure 10.21a).
     *
     * @param circle
     * @return
     */
    public boolean contains(Circle2D circle) {
        double x1 = circle.getX();
        double y1 = circle.getY();
        double x2 = getX();
        double y2 = getY();

        // Distance between 2 points
        double distance = distance(x1, y1, x2, y2);
        // Distance between the 2 radii + the radius of the contained circle 
        // <= radius of container circle
        return distance + circle.getRadius() <= getRadius();
    }

    public static double distance(double x1, double y1, double x2, double y2) {
        return Math.sqrt(Math.pow(x1 - x2, 2) + Math.pow(y1 - y2, 2));
    }

    /**
     * A method overlaps(Circle2D circle) that returns true if the specified
     * circle overlaps with this circle (see Figure 10.21c).
     *
     * @param circle
     * @return
     */
    public boolean overlaps(Circle2D circle) {
        double x1 = circle.getX();
        double y1 = circle.getY();
        double x2 = getX();
        double y2 = getY();

        // Distance between 2 points
        double distance = distance(x1, y1, x2, y2);

        return distance <= circle.getRadius() + getRadius();
    }
}
