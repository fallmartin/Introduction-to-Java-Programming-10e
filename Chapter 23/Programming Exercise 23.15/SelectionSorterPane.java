 ...
 * @author martinfall
 */
public class SelectionSorterPane extends BorderPane {
    ...
    public SelectionSorterPane() {
        ...
        btStep.setOnAction(e -> {
            int index = selectionSorter.getIndex();

            if (index >= MAX_ARRAY_SIZE) {
                btStep.setDisable(true);
            } else {
                int min = selectionSorter.iterate(index);

                if (min != index) {
                    selectionSorter.swap(min, index);
                }
            }
            ...
            selectionSorter.setIndex(index + 1);
        });
    ...
    private void paint() {
        ...
        paneForHistogram = new Histogram(selectionSorter.getIndex(),
                selectionSorter.getArray());
        setCenter(paneForHistogram);
    }
}
